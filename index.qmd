...
base_colors = {
    'mammal': '#FFE81F',        # Yellow
    'reptilian': '#00FFEA',     # Cyan
    'amphibian': '#7FFF00',     # Green
    'insectoid': '#FF5733',     # Orange
    'artificial': '#FF00FF',    # Magenta
    'unknown': '#AAAAAA'        # Gray fallback
}
...
fig.update_traces(
    ...
    insidetextfont=dict(family='Helvetica', color='white'),
    # Custom tooltip for each label
    hovertemplate=[format_tooltip(label, labels, parents) for label in labels]  
)

# Assign colors & text styles
fig.update_traces(
    marker=dict(colors=colors, line=dict(color='white', width=1)),
    insidetextfont=dict(family='Helvetica', color='black')
)
